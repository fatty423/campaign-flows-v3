  ...
  <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
      <nav class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">New Movers Campaign</h1>
...
const steps = [
  { name: 'Area Selection', path: 'map' },
  { name: 'Campaign Setup', path: 'setup' },
  { name: 'Analytics Setup', path: 'analytics' },
  { name: 'Review & Checkout', path: 'summary' }
];
...
function goToNextStep() {
  if (currentStepIndex.value < steps.length - 1) {
    const nextPath = `/new-mover/${steps[currentStepIndex.value + 1].path}`;
...
function goToPreviousStep() {
  if (currentStepIndex.value > 0) {
    const previousPath = `/new-mover/${steps[currentStepIndex.value - 1].path}`;
