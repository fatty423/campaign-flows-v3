  ...
  <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
      <nav class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Saturation Mail Campaign</h1>
...
const steps = [
  { name: 'Coverage Selection', path: 'map' },
...
function goToNextStep() {
  if (currentStepIndex.value < steps.length - 1) {
    const nextPath = `/saturation/${steps[currentStepIndex.value + 1].path}`;
...
function goToPreviousStep() {
  if (currentStepIndex.value > 0) {
    const previousPath = `/saturation/${steps[currentStepIndex.value - 1].path}`;
